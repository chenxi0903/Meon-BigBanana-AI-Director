...
export const buildQVersionThreeViewPrompt = (): string => {
  const fallback = `请将我发送给你的参考图生成为Q版角色的三视图样式，保留角色的主要特征（如发型、衣服、配饰、肤色等）。角色的外形要更加夸张可爱，头部相对较大，眼睛大而亮，身体比例缩小，整体显得更加卡通化和萌趣。请提供角色的正面、侧面和背面三视图，确保每个角度都能准确展示角色的标志性特征和个性细节`;
  const template = getEffectivePrompt('buildQVersionThreeViewPrompt', fallback);
  return applyTemplate(template, {});
};
